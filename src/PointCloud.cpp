...
bool PointCloud::loadBIN(const std::string &filename) {
  ...
  const std::size_t pointCount = static_cast<std::size_t>(fileSize / pointSize);
  ...
  for (std::size_t i = 0; i < pointCount; ++i) {
    if (!file.read(reinterpret_cast<char *>(values), sizeof(values))) {
      points.clear();
      return false;
    }

    Point point{values[0], values[1], values[2], values[3]};
